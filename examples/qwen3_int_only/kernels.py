...
def rmsnorm_q15_16_weighted(rows, cols):
    mean_shift = int(math.log2(cols))
    ...
    def main(
        ...
        with T.Kernel(rows, threads=128) as r:
            ...
            qt[0] = ((T.int32(6) + (ns[0] >> T.int32(1))) << T.int32(Q_MULTIPLIER_WIDTH)) | ((fold[0] >> T.int32(4)) & T.int32(MASK))
            for c in T.Parallel(cols):
                norm[c] = T.fix.quant(X[r, c] >> T.int32(8), scale=qt[0], out_dtype="int32") << T.int32(6)
                Y[r, c] = (norm[c] >> T.int32(8)) * (W[c] >> T.int32(8))

    return main


def rmsnorm_q15_16_weighted_safe(rows, cols, pre_shift=7):
    mean_shift = int(math.log2(cols))
    ...
    def main(
        ...
        with T.Kernel(rows, threads=128) as r:
            ...
            norm = T.alloc_fragment((cols,), "int32")
            for c in T.Parallel(cols):
                xx[0, c] = ((X[r, c] >> T.int32(8 + pre_shift)) * (X[r, c] >> T.int32(8 + pre_shift))) >> T.int32(mean_shift)
            T.reduce_sum(xx, ss, dim=1, clear=True)
            ss[0] += T.int32(1)
            ns[0] = T.int32(0)
            wk[0] = ss[0]
            ...
            qt[0] = ((T.int32(6) + (ns[0] >> T.int32(1))) << T.int32(Q_MULTIPLIER_WIDTH)) | ((fold[0] >> T.int32(4)) & T.int32(MASK))
            for c in T.Parallel(cols):
                norm[c] = T.fix.quant(X[r, c] >> T.int32(8 + pre_shift), scale=qt[0], out_dtype="int32") << T.int32(6)
                Y[r, c] = (norm[c] >> T.int32(8)) * (W[c] >> T.int32(8))
...
def shift_q15_16(rows, cols, shift):
    @T.prim_func
    def main(X: T.Tensor((rows, cols), "int32"), Y: T.Tensor((rows, cols), "int32")):
        with T.Kernel(rows, threads=128) as r:
            for c in T.Parallel(cols):
                Y[r, c] = X[r, c] >> T.int32(shift)
...
def linear_q15_16_int8_weight(rows, in_features, out_features):
    chunk_size = 64
    while in_features % chunk_size != 0:
        chunk_size //= 2
    chunks = in_features // chunk_size
    ...
    @T.prim_func
    def main(
        X: T.Tensor((rows, in_features), "int32"),
        W: T.Tensor((out_features, in_features), "int8"),
        WS: T.Tensor((out_features,), "uint32"),
        Y: T.Tensor((rows, out_features), "int32"),
    ):
        with T.Kernel(rows, out_features, threads=128) as (r, o):
            ...
            total = T.alloc_fragment((1,), "int32")
            for g, k in T.Parallel(chunks, chunk_size):
                prod[g, k] = (X[r, g * chunk_size + k] >> T.int32(8)) * T.cast(W[o, g * chunk_size + k], "int32")
            T.reduce_sum(prod, acc, dim=1, clear=True)
            T.reduce_sum(acc, total, dim=0, clear=True)
            Y[r, o] = (total[0] * T.cast(WS[o], "int32")) >> T.int32(8)

    return main


def linear_dynamic_group_q15_16(rows, in_features, out_features, groups, x_dtype="int8"):
    group_size = in_features // groups
    scale_shift = 8 if x_dtype == "int8" else 4
    acc_shift = 0 if x_dtype == "int8" else 8
    out_shift = 8 if x_dtype == "int8" else 4

    @T.prim_func
    def main(
        X: T.Tensor((rows, in_features), x_dtype),
        XS: T.Tensor((rows, groups), "uint32"),
        W: T.Tensor((out_features, in_features), "int8"),
        WS: T.Tensor((out_features,), "uint32"),
        Y: T.Tensor((rows, out_features), "int32"),
    ):
        with T.Kernel(rows, out_features, threads=128) as (r, o):
            prod = T.alloc_fragment((groups, group_size), "int32")
            acc = T.alloc_fragment((groups,), "int32")
            part = T.alloc_fragment((1, groups), "int32")
            total = T.alloc_fragment((1,), "int32")
            for g, k in T.Parallel(groups, group_size):
                prod[g, k] = T.cast(X[r, g * group_size + k], "int32") * T.cast(W[o, g * group_size + k], "int32")
            T.reduce_sum(prod, acc, dim=1, clear=True)
            for g in T.Parallel(groups):
                part[0, g] = ((acc[g] >> T.int32(acc_shift)) * T.cast((XS[r, g] * WS[o]) >> T.int32(scale_shift), "int32")) >> T.int32(out_shift)
            T.reduce_sum(part, total, dim=1, clear=True)
            Y[r, o] = total[0]

    return main


def linear_dynamic_int8_group_q15_16(rows, in_features, out_features, groups):
    return linear_dynamic_group_q15_16(rows, in_features, out_features, groups, "int8")


def linear_dynamic_int16_group_q15_16(rows, in_features, out_features, groups):
    return linear_dynamic_group_q15_16(rows, in_features, out_features, groups, "int16")


def flash_attention_i8_q15_16(batch, seqlen, dim, score_scale=1.0, v_scale=1, block_n=32):
    @T.prim_func
    def main(
        ...
        with T.Kernel(batch * seqlen, threads=128) as blk:
            ...
            for nb in T.Pipelined(seqlen // block_n):
                ...
                for j in T.Parallel(block_n):
                    sc[0, j] = T.fix.quant(red[j], scale=T.cast(QS[0], "int32"), out_dtype="int32")
                    if nb * block_n + j > i:
            ...
            for d in T.Parallel(dim):
                O[b, i, d] = T.fix.quant(acc[d], scale=qt[0], out_dtype="int32") * T.cast(VS[0], "int32")

    return main


def flash_attention_i8_q15_16_per_scale(batch, seqlen, dim, block_n=32):
    @T.prim_func
    def main(
        Q: T.Tensor((batch, seqlen, dim), "int16"),
        K: T.Tensor((batch, seqlen, dim), "int16"),
        ...
        with T.Kernel(batch * seqlen, threads=128) as blk:
            ...
            for nb in T.Pipelined(seqlen // block_n):
                ...
                for j in T.Parallel(block_n):
                    sc[0, j] = T.fix.quant(
                        red[j] >> T.int32(14),
                        scale=(((T.cast(QS[b, i, nb * block_n + j], "int32") >> T.int32(16)) - T.int32(14)) << T.int32(16))
                        | (T.cast(QS[b, i, nb * block_n + j], "int32") & T.int32(MASK)),
                        out_dtype="int32",
                    )
                    if nb * block_n + j > i:
                        sc[0, j] = T.int32(-32768)
            ...
            qt[0] = (T.int32(16) << T.int32(Q_MULTIPLIER_WIDTH)) | ((T.int32(MASK) // sm[0]) & T.int32(MASK))
            for d in T.Parallel(dim):
                O[b, i, d] = T.fix.quant(acc[d], scale=qt[0], out_dtype="int32")

    return main


def flash_attention_i8_i8_per_scale(batch, seqlen, dim, block_n=32):
    @T.prim_func
    def main(
        Q: T.Tensor((batch, seqlen, dim), "int16"),
        K: T.Tensor((batch, seqlen, dim), "int16"),
        ...
        LUT: T.Tensor((1024,), "int16"),
        QS: T.Tensor((batch, seqlen, seqlen), "uint32"),
        O: T.Tensor((batch, seqlen, dim), "int8"),
    ):
        with T.Kernel(batch * seqlen, threads=128) as blk:
            b = blk // seqlen
            i = blk - b * seqlen
            ...
            for nb in T.Pipelined(seqlen // block_n):
                ...
                T.reduce_sum(dot, red, dim=1, clear=True)
                for j in T.Parallel(block_n):
                    sc[0, j] = T.fix.quant(red[j], scale=T.cast(QS[b, i, nb * block_n + j], "int32"), out_dtype="int32")
                    if nb * block_n + j > i:
                        sc[0, j] = T.int32(-32768)
                T.reduce_max(sc, bm, dim=1, clear=True)
                nm[0] = bm[0]
            ...
            qt[0] = (T.int32(16) << T.int32(Q_MULTIPLIER_WIDTH)) | ((T.int32(MASK) // sm[0]) & T.int32(MASK))
            for d in T.Parallel(dim):
                O[b, i, d] = T.fix.quant(acc[d], scale=qt[0], out_dtype="int8")

    return main


def flash_attention_i12_i12_per_scale(batch, seqlen, dim, block_n=32):
    @T.prim_func
    def main(
        Q: T.Tensor((batch, seqlen, dim), "int16"),
        K: T.Tensor((batch, seqlen, dim), "int16"),
        ...
        with T.Kernel(batch * seqlen, threads=128) as blk:
            ...
            for nb in T.Pipelined(seqlen // block_n):
                ...
                for j in T.Parallel(block_n):
                    ...
                    sc[0, j] = T.fix.quant(
                        hi[j],
                        scale=(((T.cast(QS[b, i, nb * block_n + j], "int32") >> T.int32(Q_MULTIPLIER_WIDTH)) - T.int32(14)) << T.int32(Q_MULTIPLIER_WIDTH))
                        | (T.cast(QS[b, i, nb * block_n + j], "int32") & T.int32(MASK)),
                        out_dtype="int32",
                    )
                    sc[0, j] += T.fix.quant(lo[j], scale=T.cast(QS[b, i, nb * block_n + j], "int32"), out_dtype="int32")
                    if nb * block_n + j > i:
                        sc[0, j] = T.int32(-32768)
                T.reduce_max(sc, bm, dim=1, clear=True)
                nm[0] = bm[0]
                ...
                mx[0] = nm[0]
            for d in T.Parallel(dim):
                O[b, i, d] = T.cast(acc[d] // sm[0], "int16")
...
def packed_scale_tensor(scale, device="cuda"):
    return np.array([pack_scale(float(scale))], dtype=np.uint32)
