...
from examples.qwen3_int_only.utils.proto.common import MASK, Q15_16, fix_quant, lut_10bit, metrics, q15, rsqrt_lut_np, torch_rmsnorm
...
def proto(A, B, W, RLUT=None, qmax=32767):
    ...
    for r in range(rows):
        ...
        q = np.clip((np.abs(Y[r].astype(np.int64)) + (scale >> 1)) // scale, 0, qmax)
        q = np.where(Y[r] < 0, -q, q).astype(np.int32)
        ...
        if wk & 0xC:
            ns += 2
        inv = int(lut_10bit(ss, RLUT, ((ns - 7) << 16) | 1, "int32"))
        fold = int(fix_quant(inv, 1024.0, "int32"))
        qt = ((6 + (ns >> 1)) << 16) | ((fold >> 4) & MASK)
