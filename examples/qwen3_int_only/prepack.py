...
def main():
    ...
    parser.add_argument("--use-quarot", action="store_true")
    ...
    tensors = {}
    r1 = random_hadamard_rotation(QWEN3_0_6B.hidden_size, args.rotate_seed, args.device) if args.use_quarot else None
    r2 = random_hadamard_rotation(QWEN3_0_6B.head_dim, args.rotate_seed + 1, args.device) if args.use_quarot else None
    with safe_open(f"{args.model_dir}/model.safetensors", framework="pt", device="cpu") as f:
        ...
        for layer_idx in range(QWEN3_0_6B.num_hidden_layers):
            ...
            for name in ("q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"):
                ...
                if args.use_quarot and name in ("q_proj", "k_proj", "v_proj"):
                    weight = rotate_norm_input(weight, input_norm, r1)
                if args.use_quarot and name in ("gate_proj", "up_proj"):
                    weight = rotate_norm_input(weight, post_norm, r1)
                if args.use_quarot and name == "v_proj":
                    weight = rotate_head_output(weight, QWEN3_0_6B.head_dim, r2)
                if args.use_quarot and name == "o_proj":
                    weight = rotate_head_input(weight, QWEN3_0_6B.head_dim, r2)
                if args.use_quarot and name in ("o_proj", "down_proj"):
